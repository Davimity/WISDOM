...
class StructureResolver:
    ...
    The class owns input grammar, snapshot lookup, format labels, and source hashing. Selection
    publishes exact remote files before scientific preprocessing workers are launched.
    """
    ...
    def __init__(self, structure_dir: str | Path) -> None:
        ...
        Args:
            structure_dir: Directory containing Selection-snapshotted ``<pdb_id>.cif.gz`` files.
        """
    ...
    def resolve(
        ...
        chains ``A`` and ``Q``. A remote file must already exist in the immutable Selection
        snapshot before SHA-256 hashing. LambdaForge catches record-level failures around this
        method.
        ...
        if lower.endswith(self._LOCAL_SUFFIXES):
            ...
            if not path.is_file():
                raise FileNotFoundError(
                    f"{protein_id} is absent from the immutable Selection structure snapshot"
                )
    ...
    def output_stem(identifier: str) -> str:
        ...
        """
        lower = identifier.lower()
        if lower.endswith(StructureResolver._LOCAL_SUFFIXES):
            name = Path(identifier).name
            for suffix in StructureResolver._LOCAL_SUFFIXES:
