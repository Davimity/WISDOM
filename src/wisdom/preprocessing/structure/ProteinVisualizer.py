...
for(const id of ['surface','atoms','cartoon','normals','colour','size'])el(id).addEventListener('input',draw);window.addEventListener('resize',resize);resize();
    ...
    def run(self, context: TaskContext) -> TaskOutput:
        ...
        Returns:
            Task output declaring the visualization directory and linked HTML index as artifacts.
        ...
        return TaskOutput(
            ...
            metrics={"visualized_proteins": len(links)},
            artifacts=(
                ArtifactDeclaration(
                    path=output_dir.relative_to(context.run_dir),
                    kind=ArtifactType.REPORT,
                    media_type="text/html",
                ),
                ArtifactDeclaration(
                    path=index_path.relative_to(context.run_dir),
                    kind=ArtifactType.REPORT,
                    media_type="text/html",
                ),
            ),
