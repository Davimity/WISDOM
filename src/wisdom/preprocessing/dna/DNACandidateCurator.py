...
from wisdom.preprocessing.dna.DatasetTier import DatasetTier
...
class DNACandidateCurator(PreprocessingTransform):
    ...
    def transform(
        ...
        dna_atoms, observed_dna_chains       = self._dna_atoms(model, candidate.get("dna_chains"))
        contact_pairs, interface_residues    = self._contacts(
            protein_atoms,
            residue_ids,
            dna_atoms,
        )
        ...
        if not cluster_id or cluster_id.startswith("unassigned:"):
            exclusion = "sequence cluster is missing; use RCSB/MMseqs2 grouping before splitting"
    ...
    def _structure(self, pdb_id: str, context: TaskContext) -> Path:
