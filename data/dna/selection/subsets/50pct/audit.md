# WISDOM-DNA selection audit: 50pct

**Verdict:** `PASS_WITH_WARNINGS`. **Members:** 1390.
...
| `catalog_csv_agreement` | **PASS** | The diluted catalog contains exactly the view members and no parent-only rows. | `{"expected": 1390, "observed": 1390}` |
| `proteins_txt_agreement` | **PASS** | proteins.txt is the exact union that this view sends to preprocessing. | `{"expected": 1390, "observed": 1390}` |
| `identifiers_json_agreement` | **PASS** | identifiers.json contains one machine-readable record per labels.csv row. | `{"expected": 1390, "observed": 1390}` |
| `train_txt_agreement` | **PASS** | train.txt agrees exactly with labels.csv membership. | `{"expected": 640, "observed": 640}` |
...
| `train_class_balance` | **PASS** | Exact 1:1 parity prevents the majority class from dominating accuracy or the binary loss in this partition. | `{"negative": 320, "positive": 320, "ratio": 1.0}` |
...
| `nested_training_membership` | **PASS** | A smaller training view is a subset of every larger training view. | `{"current": 640, "previous": 320}` |
...
|---|---:|---:|---:|---:|---|
| train | 320 | 320 | 320/320 | 320/320 | `{"challenge": 40, "core": 600}` |
