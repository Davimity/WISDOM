# WISDOM-DNA selection audit: 75pct

**Verdict:** `PASS_WITH_WARNINGS`. **Members:** 1710.
...
| `catalog_csv_agreement` | **PASS** | The diluted catalog contains exactly the view members and no parent-only rows. | `{"expected": 1710, "observed": 1710}` |
| `proteins_txt_agreement` | **PASS** | proteins.txt is the exact union that this view sends to preprocessing. | `{"expected": 1710, "observed": 1710}` |
| `identifiers_json_agreement` | **PASS** | identifiers.json contains one machine-readable record per labels.csv row. | `{"expected": 1710, "observed": 1710}` |
| `train_txt_agreement` | **PASS** | train.txt agrees exactly with labels.csv membership. | `{"expected": 960, "observed": 960}` |
...
| `train_class_balance` | **PASS** | Exact 1:1 parity prevents the majority class from dominating accuracy or the binary loss in this partition. | `{"negative": 480, "positive": 480, "ratio": 1.0}` |
...
| `nested_training_membership` | **PASS** | A smaller training view is a subset of every larger training view. | `{"current": 960, "previous": 640}` |
...
|---|---:|---:|---:|---:|---|
| train | 480 | 480 | 480/480 | 480/480 | `{"challenge": 61, "core": 899}` |
