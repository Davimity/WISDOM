# WISDOM-DNA selection audit: 25pct

**Verdict:** `PASS_WITH_WARNINGS`. **Members:** 1070.
...
| `catalog_csv_agreement` | **PASS** | The diluted catalog contains exactly the view members and no parent-only rows. | `{"expected": 1070, "observed": 1070}` |
| `proteins_txt_agreement` | **PASS** | proteins.txt is the exact union that this view sends to preprocessing. | `{"expected": 1070, "observed": 1070}` |
| `identifiers_json_agreement` | **PASS** | identifiers.json contains one machine-readable record per labels.csv row. | `{"expected": 1070, "observed": 1070}` |
| `train_txt_agreement` | **PASS** | train.txt agrees exactly with labels.csv membership. | `{"expected": 320, "observed": 320}` |
...
| `train_class_balance` | **PASS** | Exact 1:1 parity prevents the majority class from dominating accuracy or the binary loss in this partition. | `{"negative": 160, "positive": 160, "ratio": 1.0}` |
...
| `nested_training_membership` | **PASS** | A smaller training view is a subset of every larger training view. | `{"current": 320, "previous": 128}` |
...
|---|---:|---:|---:|---:|---|
| train | 160 | 160 | 160/160 | 160/160 | `{"challenge": 24, "core": 296}` |
