...
class DNADatasetSink(PreprocessingSink):
    """Resolve evidence and publish homology-disjoint benchmark partitions."""
    ...
    def __init__(
        ...
        """
        if not dataset_output.strip() or not report_output.strip():
            raise ValueError("dataset and report output names cannot be empty")
    ...
    def write(self, record: PreprocessingRecord, context: TaskContext) -> None:
        ...
        record_root = context.output(self.dataset_output) / ".records"
    ...
    def is_complete(self, key: str, context: TaskContext) -> bool:
        ...
        record_path = context.output(self.dataset_output) / ".records" / record_name
        try:
    ...
    def finalize(self, context: TaskContext) -> tuple[ArtifactDeclaration, ...]:
        ...
        self._materialize_structures(accepted, dataset_root)
    ...
    def _resolve_identifiers(
        ...
        for identifier in sorted(identifier_groups):
            ...
            if len(labels) > 1:
                DNADatasetSink._mark_conflict(group, "opposing labels for one protein identifier")
        ...
        for field, description in (
            ...
            for identity, group in groups.items():
                if identity not in contradictory:
                    ...
                    continue
                DNADatasetSink._mark_conflict(group, f"opposing labels within one {description}")
                conflicts.extend(group)
            accepted = retained
    ...
    @staticmethod
    def _materialize_structures(rows: list[dict[str, Any]], dataset_root: Path) -> None:
        """Copy exact experimental structures into the portable dataset.

        Args:
            rows: Accepted rows with cached source paths and checksums.
            dataset_root: Dataset artifact root.

        Raises:
            RuntimeError: If copied bytes disagree with recorded provenance.
            OSError: If bytes cannot be copied atomically.
        """
        structure_root = dataset_root / "structures"
        structure_root.mkdir(parents=True, exist_ok=True)
        for row in rows:
            source = Path(str(row["structure_path"]))
            digest = str(row["structure_sha256"])
            suffix = "".join(source.suffixes) or ".cif"
            target = structure_root / f"{digest}{suffix}"
            ...
            if hashlib.sha256(target.read_bytes()).hexdigest() != digest:
                raise RuntimeError("portable structure digest does not match provenance")
            row["discovery_structure_path"] = str(source)
            row["structure_path"]           = target.relative_to(dataset_root).as_posix()
    ...
    def _summary(
        ...
        split_class = {
            ...
            for split in DNADatasetSink.ALL_SPLITS
        }
        rejection_reasons = Counter(
            str(row.get("exclusion_reason") or row.get("label_reason") or "unspecified")
            for row in exclusions
        )
        main_rows    = [row for row in accepted if row["split"] in DNADatasetSink.MAIN_SPLITS]
        reserve_rows = [row for row in accepted if row["split"] not in DNADatasetSink.MAIN_SPLITS]
        return {
            "verdict": "PASS",
            ...
            "feature_distributions_by_split": DNADatasetSink._distribution_statistics(accepted),
        }
    ...
    def _distribution_statistics(rows: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        output: dict[str, Any] = {}
        for split in DNADatasetSink.MAIN_SPLITS:
    ...
    def _plot(rows: list[dict[str, Any]], path: Path) -> None:
        ...
        """
        main = [row for row in rows if row["split"] in DNADatasetSink.MAIN_SPLITS]
        ...
        axes[0, 0].bar(DNADatasetSink.MAIN_SPLITS, negatives, label="negative")
        axes[0, 0].bar(
            DNADatasetSink.MAIN_SPLITS,
            positives,
            bottom=negatives,
            label="positive",
        )
