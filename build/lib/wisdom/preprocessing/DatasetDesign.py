...
class DatasetDesign(lf.Work):
    ...
    _structure_jobs_completed  : int
    _structure_jobs_interval   : int
    _structure_jobs_lock       : Lock
    _structure_jobs_total      : int
    ...
    def run(
        ...
        Raises:
            RuntimeError: If MMseqs2 or Foldseek is unavailable or unusable, structure membership
                changes, external evidence is invalid, leakage reaches multiple splits, hard
                representation constraints fail, or publication products cannot satisfy their
                scientific invariants.
        """
        # MMseqs2 and Foldseek are mandatory specialist programs, not optional fallbacks. Resolve
        # and version-probe both as the Work's first operation so an incomplete environment fails
        # before WISDOM reads evidence, transfers structures, or starts scientific computation.
        self.log(
            "External-tool preflight: resolving MMseqs2 and Foldseek before structure retrieval"
        )
        resolved_tools: list[Tool] = []
        tool_errors   : list[str]  = []
        for executable in (mmseqs_executable, foldseek_executable):
            try:
                resolved_tools.append(
                    self.tools.require(executable, version_args=["version"])
                )
            except (FileNotFoundError, RuntimeError) as error:
                tool_errors.append(f"{executable!r}: {error}")

        if tool_errors:
            details = "; ".join(tool_errors)
            message = (
                "WISDOM-DNA design stopped before structure retrieval because its external-tool "
                f"preflight failed. {details}. For a managed cluster, run "
                "'lf clusters bootstrap <cluster> --project .' from the WISDOM repository and "
                "retry after it succeeds. For local execution, run './install.sh' and activate "
                "the 'wisdom' Conda environment."
            )
            self.log(message, level="error")
            raise RuntimeError(message)

        mmseqs_tool, foldseek_tool = resolved_tools
        self.log(
            "External-tool preflight passed: "
            f"MMseqs2={mmseqs_tool.path} ({mmseqs_tool.version or 'version unavailable'}); "
            f"Foldseek={foldseek_tool.path} ({foldseek_tool.version or 'version unavailable'})"
        )
        ...
        # LambdaForge publishes aggregate map completion through ``lf top`` and includes restored
        # checkpoints in that count. This small lock-protected counter emits roughly twenty log
        # lines for jobs actually executed in this Attempt, without allowing concurrent workers to
        # race or print one message per PDB entry.
        self._structure_jobs_completed = 0
        self._structure_jobs_interval  = max(1, math.ceil(len(jobs) / 20))
        self._structure_jobs_lock      = Lock()
        self._structure_jobs_total     = len(jobs)

        self.log(
            f"Retrieving or reusing and analysing {len(jobs)} unique PDB entries "
            f"with {workers} bounded workers; live aggregate progress is available in lf top"
        )
        ...
        #
        # Both specialist executables were resolved before Phase 2. LambdaForge records their
        # absolute paths and versions in environment provenance; there is no approximate fallback.
    ...
    def _analyse_pdb(self, job: Mapping[str, Any]) -> dict[str, Any]:
        ...
        # ``resume_map`` updates the authoritative aggregate counter after this callback returns.
        # This additional Work log reports only callbacks executed in the current Attempt; restored
        # map records can make the aggregate ``lf top`` count larger. A cache hit still validates
        # the managed bytes but performs no HTTP request.
        with self._structure_jobs_lock:
            self._structure_jobs_completed += 1
            completed = self._structure_jobs_completed
            report = (
                completed == self._structure_jobs_total
                or completed % self._structure_jobs_interval == 0
            )
            if report:
                self.log(
                    "Structure retrieval and analysis executed in this Attempt: "
                    f"{completed}/{self._structure_jobs_total} PDB entries; "
                    "valid managed-cache entries were reused without downloading"
                )
