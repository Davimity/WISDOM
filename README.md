...
- [3. DNA-binding benchmark and annotations](#3-dna-binding-benchmark-and-annotations)
  - [3.1. Selection Task and dataset recipe](#31-selection-task-and-dataset-recipe)
...
- [5. Trainable WISDOM models](#5-trainable-wisdom-models)
  - [5.1. Label manifest and graph batching](#51-label-manifest-and-graph-batching)
  ...
  - [5.4. Training, post-run evaluation, and 3D artifacts](#54-training-post-run-evaluation-and-3d-artifacts)
- [6. Bibliography](#6-bibliography)

## 1. Quick start

The production path has two deliberately separate executions. First,
[`experiments/dna_select.yaml`](experiments/dna_select.yaml) is an ordinary resumable Task that
...
[`experiments/dna_preprocess.yaml`](experiments/dna_preprocess.yaml) consumes that frozen selection,
...
`validate` checks a Task or the recipe and every embedded Task. `datasets plan` reports whether each
heavy stage will execute or reuse a verified content-addressed result. `run` executes the selected
configuration and, for the recipe, publishes one immutable placement only after every required
stage and the canonical index validate. `lf` and `lambdaforge` are equivalent command names. A local
...
`members.jsonl` is the authoritative streaming index: each line names one protein, its split and
tier, its global DNA target, whether local ground truth is available, and checksummed base/sidecar
assets. `dataset-artifact.json` stores the path-independent content ID and separate build provenance.
...
- LambdaForge `>=0.10.0,<0.11`, normally installed from its local checkout;
- a CPU environment with NumPy, SciPy, and Gemmi;
- Internet access only when remote PDB entries are absent from the raw cache.

WISDOM targets LambdaForge `0.10.0`. LambdaForge is the source of truth for Task materialization,
named inputs and outputs, preprocessing iteration and resume, dataset recipes, canonical indexes,
immutable versions, atomic publication, the placement Registry, logging, resources, and run
...
LambdaForge's `PreprocessingTask` executes each WISDOM source→transform→sink stage. A
`DatasetRecipe` compiles those Tasks onto a dependency graph, reuses exact successful stages, and
publishes only the selected final root. WISDOM does not implement a second scheduler, process pool,
stage cache, Registry, or publisher. Section 4.7 explains this boundary after the scientific data
flow is established.

## 3. DNA-binding benchmark and annotations

### 3.1. Selection Task and dataset recipe
...
[`experiments/dna_select.yaml`](experiments/dna_select.yaml) is therefore an ordinary resumable Task.
...
[`experiments/dna_preprocess.yaml`](experiments/dna_preprocess.yaml) consumes that frozen artifact
later. Its `geometry` stage creates universal, label-free structural NPZ files from
`selection/proteins.txt`. Its `annotate` stage joins the frozen catalog to the exact geometry report,
reuses geometry's coordinate cache, creates aligned DNA sidecars, and writes `members.jsonl`. Only
this recipe publishes `wisdom-dna@2`; a failed build publishes no `DatasetVersion`.

This separates four LambdaForge 0.10.0 concepts. A **recipe** describes how to rebuild the data. A
**build** is one execution with stage fingerprints and provenance. A **version** is the immutable
...
LambdaForge 0.10 makes `lf run` the canonical entry point for both Tasks and dataset recipes. The
recipe's top-level `resources` block is an exact request for 36 CPU, 128 GiB, no GPU, and 24 hours;
no CLI resource override is needed. `lf datasets build` remains a compatibility alias.
...
# Then plan and run the expensive immutable dataset build.
lf validate experiments/dna_preprocess.yaml
lf datasets plan experiments/dna_preprocess.yaml --verbose
lf run experiments/dna_preprocess.yaml --dry-run
...
`catalog.csv`, `identifiers.json`, `proteins.txt`, main/reserve TXT lists, labels, and diluted views.
Large candidate checkpoints and discovery downloads are separate Task outputs and are intentionally
not fetched. The recipe fingerprints the fetched selection as an input: changed membership
invalidates geometry and annotation, while an identical selection allows `reuse: auto` to reuse both
exact verified stages. It never trusts an arbitrary output merely because a directory exists.
...
its label, split, 30%-identity cluster, tier, and logical-member flag. The final stage copies these
contracts into the published dataset. Geometry declares its parallel `raw` coordinate cache as an
artifact binding for annotation, so selected structures are not downloaded twice.
...
LambdaForge 0.10.0 writes preprocessing progress to stderr, which becomes the durable job log. One
...
Exact sequences and homologous proteins must not leak between train, validation, and test. WISDOM
uses official RCSB MMseqs2 30%-identity groups and never runs a project-owned mass aligner. DyProL
and BTD published test records stay in `test`; they can never enter train/validation. Complete
development clusters are assigned approximately 80/20 to train/validation by a seeded SHA-256
ordering. If a development cluster overlaps external test, only its external records remain.
Positive clusters are also held in `validation_reserve` and `test_reserve`; reserves never train.
...
fails. This guarantees binary class parity; it does not claim demographic, taxonomic, or structural
distributional fairness, which remains visible in the report. Geometry and annotation also use
`on_error: fail`, so a selected member cannot silently disappear and break parity downstream.
...
chains, and binding-residue indices where applicable. Minimal `train.txt`, `validation.txt`,
`test.txt`, reserve TXT files, and `proteins.txt` contain only `PDBID_CHAINS` selectors.
`identifiers.json` provides the same selection with label, split, cluster, tier, and explicit
main-member/reserve status. The JSON report gives candidate/accepted/rejected counts by class,
reasons—including majority downsampling—PDB-mapping failures, exact class balance, cluster/source
statistics, and structural distributions.
...
Reserves are deliberately absent from this manifest and from `members.jsonl`; their files exist
...
25%, 50%, and 75%—under `subsets/<fraction>/`. If split (s) contains (N_{sc}) examples of class
(c), fraction (f) retains

\[
n_{sc}(f)=\max\left(1,\left\lfloor fN_{sc}\right\rfloor\right)
\]

for each class independently. The full splits are already balanced, so every view stays balanced.
Rows are ordered breadth-first across external 30%-identity clusters: one member of each cluster is
visited before a cluster contributes a second. Seeded ordering makes the views deterministic and
nested: 10% is contained in 25%, then 50%, 75%, and the full selection. Original split ownership is
never changed, so the cross-split homology barrier remains intact. The heavy recipe preprocesses the
union once; its annotation sink turns each `subsets/<fraction>/identifiers.json` into a lightweight
`subsets/<fraction>/manifest.csv` that references the shared NPZ/sidecar files. To run one view,
change all three experiment loaders' `subpath` to, for example,
`subsets/25pct/manifest.csv`; their `split` parameters still select train, validation, and test.
...
The final `members.jsonl` expresses the same main-split collection in LambdaForge's canonical
...
`dna_annotation`, and selected `source_structure` assets. `catalog.csv`, all manifests, the split
TXT files, `identifiers.json`, and the annotation report are checksummed global assets. Consequently
the content ID depends on scientific
...
The `geometry` stage inside
[`experiments/dna_preprocess.yaml`](experiments/dna_preprocess.yaml) is the human-editable structural
description. YAML is a configuration format; this embedded Task selects the bound input manifest,
output directories, scientific parameters, and execution policy. LambdaForge materializes it as one
content-addressed recipe stage.
...
`validate` catches malformed recipe/stage configuration and unavailable Python classes. The verbose
plan shows `EXECUTE`, `REUSE`, or `MISSING` for every stage and `PUBLISH` or `NOOP` for the final
boundary. `--dry-run` submits nothing and transforms no proteins. The final command performs the
...
|---|---:|---|
| `geometry.inputs.protein_identifiers` | `data/dna/selection/proteins.txt` | Exact manifest fetched from the independent selection Task. |
| `outputs.downloads` | `raw` | Named run-relative cache for downloaded `cif.gz` files. |
| `outputs.processed` | `processed` | Named run-relative NPZ dataset directory. |
| `outputs.report` | `preprocessing-report.json` | Named WISDOM scientific/compatibility report. |
| `task.params.workers` | `36` | One concurrent CPU worker per core requested by the production `lf run` command. |
| `task.params.workload` | `cpu` | Use spawned processes for CPU transforms; the parent retains sink and manifest writes. |
| `task.params.on_error` | `fail` | Stop if a selected balanced member cannot produce its heavy representation. |
| `task.params.checkpoint_interval` | `1` | Persist LambdaForge record progress after every completed protein. |
| `task.params.progress_interval_seconds` | `10.0` (framework default) | Emit a coordinator-owned aggregate progress line at eligible checkpoints about every ten seconds. |
| `transform.download` | `true` | Permit RCSB download when a remote entry is absent from the named cache. |
| `resources` in recipe | `36 CPU, 128 GiB, 24 h` | Exact outer allocation shared sequentially by geometry and annotation. |
| `model_index` | `0` | Zero-based structural model selected from the coordinate file. |
...
LambdaForge 0.10.0 can safely inspect, constrain, and render explicit arrays without WISDOM-specific
...
**Stage and whole-recipe reuse.** A normal second invocation of the same successful recipe is a
`NOOP`, not another preprocessing run. For each stage LambdaForge requires the same Task fingerprint,
a successful result, every required path, and current artifact digests equal to those recorded at
completion. WISDOM additionally revalidates resumed molecular records at their sink boundaries.
Thus
...
shows `REUSE` for all stages and `NOOP` for publication when nothing relevant changed. In that case
LambdaForge neither downloads structures nor rebuilds surfaces. Changing declared input bytes, code
identity, or a fingerprinted scientific setting invalidates that stage and every downstream stage.
Changing published scientific content also requires a new explicit dataset version because an
existing `name@version` is immutable.
...
In LambdaForge 0.10.0, the recipe-level resource block determines the real enclosing reservation:

```bash
lf run experiments/dna_preprocess.yaml --on citius-ctgpgpu12
```

The verified dry-run reports `CPU=36`, 128 GiB, no GPU, and 24 hours. `processes: 1` in that plan is
not the number of protein workers and does not reduce execution to one core: it means one outer
dataset-build coordinator rather than multiple distributed ranks. Inside that coordinator, each
`PreprocessingTask` creates its configured 36-process pool. The two recipe stages are sequential, so
both reuse the same 36-core reservation instead of requiring 72 cores. Do not use 72 CPU-bound
...
For Task, Experiment, and LambdaForge 0.10 dataset-recipe YAMLs, a top-level `resources` block
requests CPU, RAM, GPU, process ranks, and time portably. After a cluster profile such as `atlas` is
...
With LambdaForge 0.10.0, each `environment: managed` cluster profile should normally leave PyTorch
selection automatic:
...
The code mirrors the journey rather than placing every operation in one large function.
LambdaForge owns the generic task shell; `PreprocessPipeline` reads like the one-protein sequence in
Section 4.1, while each peripheral class owns one cohesive kind of complexity:

`src/preprocess` uses one class per class file and no free module functions. Static chemical tables
remain in `chemical_data.py` rather than being wrapped in artificial state.
...
### 5.1. Label manifest and graph batching

Preprocessing does not know experimental labels or train/validation/test membership. Training adds
them through the smallest explicit CSV contract:
...
`file` is an NPZ path relative to the manifest, `label` is exactly `0` or `1`, and `split` is exactly
`train`, `val`, or `test`. Explicit splits prevent the same protein from leaking between training
and evaluation. `WisdomDataset` opens each NPZ with `allow_pickle=False`, checks the arrays and graph
ranges needed by v1, and converts only those arrays to tensors. It does not move points, recompute
edges, or mutate preprocessing output.
...
### 5.4. Training, post-run evaluation, and 3D artifacts

WISDOM contains no HPO scheduler, pruning loop, seed manager, GPU selector, or study database.
LambdaForge owns model materialization, Sobol initialization, Bayesian search, cost-aware knowledge
gradient, cumulative-epoch fidelity, learning-curve decisions, pruning, adaptive seed racing,
confirmation seeds, checkpoints, memory admission, persistence, resume, aggregation, reports, and
result indexing. WISDOM contributes its dataset, graph collator, models, scientific search spaces,
and regional pooling semantics.

| Configuration | Responsibility |
|---|---|
| `wisdom_v1.yaml` | Complete v1 model, training contract, and adaptive backbone HPO with atom→surface mean and MAX fixed. |
| `wisdom_v2.yaml` | Complete v2 model, the selected v1 backbone values, and conditional HPO exclusively over pooling. |

V1 optimizes validation AUPRC, never test. Sobol `trials:auto` derives the initial design size from
the effective dimension; Bayesian cost-aware knowledge gradient follows. A candidate begins at five
epochs and may resume in five-epoch increments to 100. Posterior pruning starts only after ten
epochs. Search seeds `[7,17,27]` are raced adaptively, while the three finalist configurations use
new confirmation seeds `[101,211]`. Hard limits are 60 actions, 2,000 total epochs, and 200,000
GPU-seconds. They bound cost rather than requiring LambdaForge to consume the whole budget.

V2 uses one conditional HPO. It compares MAX, mean, attention, five top-k fractions, one to three
regional levels, and log-sum-exp. A top-k fraction exists only for `topk`, region levels only for
`local_mean_max`, attention width only for `attention`, and beta only for log-sum-exp. Backbone
features, widths, graph depths, dropout, learning rate, and weight decay are absent from the v2
search: all are copied from the robust v1 selection and remain controlled constants.

The three split loaders refer to
`{dataset: wisdom-dna, version: "2", subpath: manifest.csv}`, not to a machine-specific absolute
path. LambdaForge resolves `wisdom-dna@2` through DatasetRegistry and records the exact content/build
...
Install LambdaForge's `adaptive-hpo` extra before using Bayesian search. Start v1 with the normal
command. Repeating the exact command resumes its durable study from
`.lambdaforge/adaptive/<study-id>/`; `--restart` explicitly discards progress. Never edit its
`state.json`, append-only events, or `summary.json` manually.
...
Review confirmation means, dispersion, learning curves, suspicious search boundaries, and model
simplicity; do not copy the largest decimal blindly. Then copy the selected backbone and optimizer
values into the clearly marked fixed blocks of `wisdom_v2.yaml` and run its pooling HPO:
...
The v2 YAML carries a `scientific_status` warning until that manual copy is complete. The checked
workstation has one 4-GB GPU, so HPO concurrency is one and LambdaForge reserves 3 GiB plus
headroom with a defensive allocator cap. These are operational settings, not scientific parameters;
a managed target may override resources without changing candidate identity. LambdaForge's generic
NPZ inspection and explicit-role 3D visualization commands remain documented in Section 4.2.
WISDOM retains only domain validation that generic tooling cannot infer, such as protein topology,
signed surface gaps, normal orientation, and curvature identities.

Final DNA analysis is a required LambdaForge `post_run` action executed on the same allocation after
successful training has been committed. In these adaptive YAMLs, `scope: confirmed_runs` restricts
the expensive test-set metrics and 3D exports to the finalist configurations retrained with the
independent confirmation seeds. A non-adaptive experiment would run the same action after a normal
completion or trainer early stopping. Search trials that are paused, pruned, cancelled, failed, or
cooperatively interrupted do not run post-run actions. `scope: all_runs` can include every
successful terminal search run, but it still does not turn any of those excluded states into a
post-run state.

The YAML explicitly requests `checkpoint: best`; LambdaForge does not silently substitute `last`.
If an unambiguous best checkpoint is absent, the required action fails visibly while the committed
training checkpoint remains available for diagnosis and a post-run-only retry. The action uses the
persisted checkpoint SHA-256, its own configuration identity, and declared artifact hashes to write
a durable receipt. Repeating an identical completed run reuses a verified receipt; changing the
evaluation policy or losing an artifact reruns only the affected action, not neural training.
Surface ground truth never enters the loss, gradients, validation AUPRC, HPO policy, or model
selection.

Global test reporting uses LambdaForge's accuracy, balanced accuracy, precision, recall,
specificity, F1, MCC, Cohen's kappa, AUROC, and AUPRC. Surface reporting applies the same binary
family to points where `surface_valid_mask` is true. Scores are
(\sigma(\ell_i)=1/(1+e^{-\ell_i})), where (ell_i) is a local model logit and the default discrete
threshold is 0.5. Micro metrics concatenate valid points; macro metrics first evaluate each protein
and then average only defined values. The report separates all-protein summaries from primary
positive-protein localization and recomputes positive metrics at every stored DNA-gap sensitivity
cutoff without retraining. Reports preserve evaluated-protein and undefined counts; an undefined
denominator is `null`, never zero. Positive proteins additionally report Dice and IoU.
Curated negatives separately report predicted-positive area fraction, maximum probability, number
of spurious connected hotspots, and largest hotspot size.

Post-run artifacts follow the training run rather than a second results hierarchy:
...
PLY is a standard point-cloud format. Each vertex retains the same `(x,y,z)` as the base surface and
may expose normals, area, each curvature invariant/scale, hard/soft target, validity, DNA distance,
surface logit, and surface probability as selectable scalar properties. The companion NPZ preserves
the same channels without text precision loss. Learned dimensions are named
`latent_chemical_channel_N`, not electrostatic charge or another physical quantity they were never
trained to represent. Only explicitly configured latent indices are exported; PCA is never applied
silently and visual export never changes metric values.
